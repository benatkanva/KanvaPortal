...
function SchemaMapperContent() {
  ...
  const loadCompleteSchema = useCallback(async () => {
    setLoading(true);
    try {
      // Load the complete schema config generated by inspect-schema
      const response = await fetch('/docs/schema-mapper-config.json');
      if (response.ok) {
        ...
        console.log(`✅ Loaded ${collections.length} collections with real fields`);
      } else {
        console.error('Schema config not found. Run: npm run inspect-schema');
      }
    } catch (err) {
      console.error('Error loading schema:', err);
      console.error('Make sure to run: npm run inspect-schema');
    } finally {
      setLoading(false);
    }
  }, []);
